...
CM <- read.csv("Data/edi_df_integrate_monthly.csv")
...
I <- which(result=="TRUE")
p1<-ggplot()+geom_path(data=test,aes(x=Date,y=Chl, group=1),color="black")+
...
p7<-ggplot()+geom_path(data = test,aes(x=Date,y=Chl, group=1),color="black")+
...
p13<-ggplot()+geom_path(data = test,aes(x=Date,y=Chl, group=1),color="black")+
  geom_point(data = test %>% filter(bloom=="Yes"), aes(x = Date, y = Chl),color="blue")+ggtitle("Suisun Marsh")
...
p19<- ggplot()+
  ...
  geom_point(data=x25[r,],aes(x=Date,y=Chl, group=1),color="blue")+ggtitle("Suisun Marsh")
...
p25<- ggplot()+
  ...
  geom_path(data=x25,aes(x=Date,y=Chl, group=1),color="black")+
  geom_point(data=x25%>% filter(Bloom=="Yes"),aes(x=Date,y=Chl, group=1),color="blue")+ggtitle("Suisun Marsh")
...
p31 <- ggplot()+
  ...
  geom_point(data=test_pi%>% filter(bloom=="Yes"),aes(x=Date,y=Chl, group=1),color="blue")+ggtitle("Suisun Marsh")
...
library(scales)
# adjust date formatting for the plots
p1 +
  scale_x_date(
    labels = label_date_short(),  # auto formatted labels
    date_breaks = "10 years"      # adjust tick frequency
  ) +
...
tiff("Figures/Bloom detection discrete/MethodComp_SuisunMarsh.tiff", units="in", width=16, height=9, res=300)
